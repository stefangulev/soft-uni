...
import com.example.mymusic.repositories.ArtistRepository;
...
public class ArtistServiceImpl implements ArtistService {
    ...
    public void initializeArtists() {
        if (artistRepository.count() == 0) {
            Arrays.stream(ArtistNameEnum.values()).forEach(a -> {
                Artist artist = new Artist().setName(a);
                switch (a) {
                    case QUEEN:
    ...
    @Override
    public Artist findArtistByName(ArtistNameEnum artistNameEnum) {
        return artistRepository.findArtistByName(artistNameEnum).orElse(null);
    }
}
